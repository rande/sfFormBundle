...
$w1 = new InputText(array(), array('class' => 'foo1'));
$w2 = new InputText();
...
$w = new Schema(array(), array('form_formatter' => 'list'));
...
$w = new Schema(array(), array('name_format' => 'name_%s'));
...
$w['w2'] = $w2;
$w1->setParent($w); $w2->setParent($w);
$t->ok($w->getFields() == array('w1' => $w1, 'w2' => $w2), 'Schema implements the \ArrayAccess interface for the fields');
...
{
  $w['w1'] = 'string';
  $t->fail('Schema implements the \ArrayAccess interface for the fields');
}
catch (\LogicException $e)
{
  $t->pass('Schema implements the \ArrayAccess interface for the fields');
}

$w = new Schema(array('w1' => $w1));
$t->is(isset($w['w1']), true, 'Schema implements the \ArrayAccess interface for the fields');
$t->is(isset($w['w2']), false, 'Schema implements the \ArrayAccess interface for the fields');

$w = new Schema(array('w1' => $w1));
$w1->setParent($w); $w2->setParent($w);
$t->ok($w['w1'] == $w1, 'Schema implements the \ArrayAccess interface for the fields');
$t->is($w['w2'], null, 'Schema implements the \ArrayAccess interface for the fields');

$w = new Schema(array('w1' => $w1, 'w2' => $w2));
unset($w['w1']);
$t->is($w['w1'], null, 'Schema implements the \ArrayAccess interface for the fields');
$t->is($w->getPositions(), array('w2'), 'Schema implements the \ArrayAccess interface for the fields');

// unset with numeric keys
$w = new Schema(array('0' => $w1, 'w2' => $w2));
unset($w['w2']);
$t->is($w['w2'], null, 'Schema implements the \ArrayAccess interface for the fields');
$t->is($w->getPositions(), array('0'), 'Schema implements the \ArrayAccess interface for the fields');

$w = new Schema(array('w1' => $w1, '0' => $w2));
unset($w[0]);
$t->is($w[0], null, 'Schema implements the \ArrayAccess interface for the fields');
$t->is($w->getPositions(), array('w1'), 'Schema implements the \ArrayAccess interface for the fields');
...
$w = new Schema(array(
  'author' => new Schema(array(
    'first_name' => new InputText(),
    'company'    => new Schema(array(
      'name' => new InputText(),
    )),
  )),
));
...
$author = new Schema(array('first_name' => new InputText()));
$company = new Schema(array('name' => new InputText()));
...
$w = new Schema(array('first_name' => new InputText()));
...
// ->renderField()
$t->diag('->renderField()');
$w = new Schema(array('first_name' => $w1));
...
$w = new Schema(array('first_name' => $ww));
...
$w = new Schema(array('w1' => $w1, 'w2' => $w2, '0' => $w1));
$w->moveField('w1', Schema::BEFORE, '0');
...
$w = new Schema();
$w['w1'] = $w1;
$w['w2'] = new InputHidden();
$w['w3'] = new Schema();
$w['w3']['w1'] = $w1;
$w['w3']['w2'] = new InputHidden();
$errors = array(
  'global error',
  'w1' => 'error for w1',
  'w2' => 'error for w2',
...
$w = new Schema(array('w1' => new InputHidden()));
...
$w = new Schema();
$w['w1'] = new InputHidden();
$ew = new Schema(array('w3' => new InputText()));
$w['w4'] = new SchemaDecorator($ew, $w->getFormFormatter()->getDecoratorFormat());
$expected = <<<EOF
<tr>
  <th>W4</th>
  <td>
...
$w = new Schema(array('w1' => $w1, 'w2' => $w2));
$w1->setParent($w); $w2->setParent($w);
$format1 = new SchemaFormatterList($w);
...
$t->is(count($formatters), 1 , '__clone() returns a Schema that has the Formatters attached');
...
$w = new Schema();
$w->addFormFormatter('table', new SchemaFormatterTable($w));
$w->addFormFormatter('list', new SchemaFormatterList($w));
$w1 = clone $w;
