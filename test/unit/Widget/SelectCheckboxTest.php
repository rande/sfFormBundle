...
use Bundle\sfFormBundle\Widget\SelectCheckbox;
...
// ->render()
$t->diag('->render()');
$w = new SelectCheckbox(array('choices' => array('foo' => 'bar', 'foobar' => 'foo'), 'separator' => ''));
...
$w = new SelectCheckbox(array('choices' => array('0' => 'bar', '1' => 'foo')));
$output = <<< EOF
...
$w = new SelectCheckbox(array('choices' => array()));
$t->is($w->render('myname', array()), '', '->render() returns an empty HTML string if no choices');

// group support
$t->diag('group support');
$w = new SelectCheckbox(array('choices' => array('foo' => array('foo' => 'bar', 'bar' => 'foo'), 'bar' => array('foobar' => 'barfoo'))));
$output = <<<EOF
...
$w = new SelectCheckbox(array('choices' => new Callable('choice_callable')));
$dom->loadHTML($w->render('foo'));
$css = new DomCssSelector($dom);
$t->is(count($css->matchAll('input[type="checkbox"]')->getNodes()), 3, '->render() accepts a Callable as a choices option');
...
$w = new SelectCheckbox(array('choices' => array('<b>Hello world</b>')));
...
// __clone()
$t->diag('__clone()');
$w = new SelectCheckbox(array('choices' => new Callable(array($w, 'foo'))));
$w1 = clone $w;
...
$w = new SelectCheckbox(array('choices' => new Callable(array($a = new stdClass(), 'foo'))));
$w1 = clone $w;
$callable = $w1->getOption('choices')->getCallable();
