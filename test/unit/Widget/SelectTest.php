...
use Bundle\sfFormBundle\Widget\Select;
...
// ->render()
$t->diag('->render()');
$w = new Select(array('choices' => array('foo' => 'bar', 'foobar' => 'foo')));
...
$w = new Select(array('choices' => array('' => 'bar')));
...
$w = new Select(array('multiple' => true, 'choices' => array('foo' => 'bar', 'foobar' => 'foo')));
...
$w = new Select(array('choices' => array('foo' => array('foo' => 'bar', 'bar' => 'foo'), 'foobar' => 'foo')));
...
$ws->addFormFormatter('stub', new FormFormatterStub());
$ws->setFormFormatterName('stub');
$w = new Select(array('choices' => array('group' => array('foo' => 'bar', 'foobar' => 'foo'))));
$w->setParent($ws);
$dom->loadHTML($w->render('foo'));
$css = new DomCssSelector($dom);
...
$w = new Select(array('choices' => new Callable('choice_callable')));
$dom->loadHTML($w->render('foo'));
$css = new DomCssSelector($dom);
$t->is(count($css->matchAll('#foo option')->getNodes()), 3, '->render() accepts a Callable as a choices option');

// attributes
$t->diag('attributes');
$w = new Select(array('choices' => array(0, 1, 2)));
...
$w = new Select(array('choices' => array(0, 1, 2)), array('disabled' => 'disabled'));
$dom->loadHTML($w->render('foo'));
$css = new DomCssSelector($dom);
$t->is(count($css->matchAll('select[disabled="disabled"]')->getNodes()), 1, '->render() does not pass the select HTML attributes to the option tag');
$t->is(count($css->matchAll('option[disabled="disabled"]')->getNodes()), 0, '->render() does not pass the select HTML attributes to the option tag');
...
// __clone()
$t->diag('__clone()');
$w = new Select(array('choices' => new Callable(array($w, 'foo'))));
$w1 = clone $w;
...
$w = new Select(array('choices' => new Callable(array($a = new stdClass(), 'foo'))));
$w1 = clone $w;
$callable = $w1->getOption('choices')->getCallable();
