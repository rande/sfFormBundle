...
class PreValidator extends Base
{
  protected function doClean($values)
  {
    if (isset($values['s1']) && isset($values['s2']))
    {
      throw new Error($this, 's1_or_s2', array('value' => $values));
    }
  }
}

class PostValidator extends Base
{
  protected function doClean($values)
  {
...
class Post1Validator extends Base
{
  protected function doClean($values)
  {
    if ($values['s1'] == $values['s2'])

    throw new Error($this, 's1_not_equal_s2', array('value' => $values));
  }
}
...
$t->is($v->getFields(), array('s1' => $v1, 's2' => $v2), '->__construct() can take an array of named  objects');
try
{
  $v = new Schema('string');
...
$v['s1'] = $v1;
$v['s2'] = $v2;
$t->is($v->getFields(), array('s1' => $v1, 's2' => $v2), 'Schema implements the \ArrayAccess interface for the fields');
...
try
{
  ...
  $t->fail('->clean() throws an ErrorSchema exception if a you give a non existant field');
  $t->skip('', 2);
}
catch (ErrorSchema $e)
{
  $t->pass('->clean() throws an ErrorSchema exception if a you give a non existant field');
...
try
{
  ...
  $t->fail('->clean() throws an ErrorSchema exception if a required field is not provided');
  $t->skip('', 2);
}
catch (ErrorSchema $e)
{
  $t->pass('->clean() throws an ErrorSchema exception if a required field is not provided');
  $t->is(count($e), 1, '->clean() throws an exception with all error messages');
...
$v1 = new String(array('max_length' => 3, 'required' => false));
$v2 = new String(array('min_length' => 3, 'required' => false));
$v = new Schema(array('s1' => $v1, 's2' => $v2));
...
try
{
  ...
  $t->fail('->clean() throws an ErrorSchema exception if a pre-validator fails');
  $t->skip('', 2);
}
catch (ErrorSchema $e)
{
  $t->pass('->clean() throws an ErrorSchema exception if a pre-validator fails');
  $t->is(count($e), 1, '->clean() throws an exception with all error messages');
...
try
{
  ...
  $t->fail('->clean() throws an ErrorSchema exception if a post-validator fails');
  $t->skip('', 2);
}
catch (ErrorSchema $e)
{
  $t->pass('->clean() throws an ErrorSchema exception if a post-validator fails');
  $t->is(count($e), 1, '->clean() throws an exception with all error messages');
...
$comparator = new SchemaCompare('left', SchemaCompare::EQUAL, 'right');
$userValidator = new Schema(array(
  'test'  => new String(array('min_length' => 10)),
  'left'  => new String(array('min_length' => 2)),
  'right' => new String(array('min_length' => 2)),
));
...
$comparator1 = new SchemaCompare('password', SchemaCompare::EQUAL, 'password_bis');
$v = new Schema(array(
  'left'         => new String(array('min_length' => 2)),
  'right'        => new String(array('min_length' => 2)),
  'password'     => new String(array('min_length' => 2)),
  'password_bis' => new String(array('min_length' => 2)),
));
...
$v->setPreValidator(new String(array('min_length' => 4)));
$v->setPostValidator(new String(array('min_length' => 4)));
$v1 = clone $v;
