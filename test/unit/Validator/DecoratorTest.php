...
class MyValidator extends Decorator
{
  public function getValidator()
  {
    return new String(array('min_length' => 2, 'trim' => true), array('required' => 'This string is required.'));
  }
}
...
{
  $v->clean(null);
  $t->fail('->clean() throws a Error if the value is required');
  $t->skip('', 1);
}
catch (Error $e)
{
  $t->pass('->clean() throws a Error if the value is required');
...
{
  $v->clean('f');
  $t->fail('->clean() throws a Error if the wrapped validator failed');
  $t->skip('', 1);
}
catch (Error $e)
{
  $t->pass('->clean() throws a Error if the wrapped validator failed');
  $t->is($e->getCode(), 'min_length', '->clean() throws a Error');
}

$t->is($v->clean('  foo  '), 'foo', '->clean() cleans the value by executing the clean() method from the wrapped validator');

class FakeValidator extends Decorator
{
...
{
  $v = new FakeValidator();
  $t->fail('->clean() throws a \RuntimeException if getValidator() does not return a  instance');
}
catch (\RuntimeException $e)
{
  $t->pass('->clean() throws a \RuntimeException if getValidator() does not return a  instance');
}
