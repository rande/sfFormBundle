...
class ValidatorIdentity extends Base
{
...
class ValidatorIdentityWithRequired extends Base
{
  protected function configure($options = array(), $messages = array())
  {
...
{
  $t->is($v->clean(''), '');
  $t->fail('->clean() throws a Error exception if the data does not validate');
  $t->skip('', 1);
}
catch (Error $e)
{
  $t->pass('->clean() throws a Error exception if the data does not validate');
