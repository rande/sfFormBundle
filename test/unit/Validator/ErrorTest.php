...
$e = new Error($v, 'max_length', array('value' => 'foo<br />', 'max_length' => 1));

// ->getValue()
$t->diag('->getValue()');
$t->is($e->getValue(), 'foo<br />', '->getValue() returns the value that has been validated with the validator');

$e1 = new Error($v, 'max_length', array('max_length' => 1));
...
{
  return serialize(new Error(new String(), 'max_length', array('value' => 'foo<br />', 'max_length' => 1)));
}
...
try
{
  ...
  $t->pass('Error implements \Serializable');
}
catch (Exception $e)
{
  $t->fail('Error implements \Serializable');
}

$e1 = unserialize($serialized);
$t->is($e1->getMessage(), $e->getMessage(), 'Error implements \Serializable');
$t->is($e1->getCode(), $e->getCode(), 'Error implements \Serializable');
$t->is(get_class($e1->getValidator()), get_class($e->getValidator()), 'Error implements \Serializable');
$t->is($e1->getArguments(), $e->getArguments(), 'Error implements \Serializable');
