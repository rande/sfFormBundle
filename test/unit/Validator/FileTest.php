...
class testValidatorFile extends File
{
...
try
{
  ...
  $t->fail('->clean() throws an Error if the given value is not well formatted');
  $t->skip('', 1);
}
catch (Error $e)
{
  $t->pass('->clean() throws an Error if the given value is not well formatted');
  $t->is($e->getCode(), 'invalid', '->clean() throws a Error');
}
$f = $v->clean(array('tmp_name' => $tmpDir.'/test.txt'));
$t->ok($f instanceof ValidatedFile, '->clean() returns a ValidatedFile instance');
$t->is($f->getOriginalName(), '', '->clean() returns a ValidatedFile with an empty original name if the name is not passed in the initial value');
$t->is($f->getSize(), strlen($content), '->clean() returns a ValidatedFile with a computed file size if the size is not passed in the initial value');
// $t->is($f->getType(), (version_compare(PHP_VERSION, '5.3', '<')) ? 'text/plain' : 'text/plain; charset=us-ascii', '->clean() returns a ValidatedFile with a guessed content type');
$t->is($f->getType(), 'text/plain', '->clean() returns a ValidatedFile with a guessed content type');

class myValidatedFile extends ValidatedFile
{
}
...
foreach (array(UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE, UPLOAD_ERR_PARTIAL, UPLOAD_ERR_NO_TMP_DIR, UPLOAD_ERR_CANT_WRITE, UPLOAD_ERR_EXTENSION) as $error)
{
  try
  {
    ...
    $t->fail('->clean() throws an Error if the error code is not UPLOAD_ERR_OK (0)');
    $t->skip('', 1);
  }
  catch (Error $e)
  {
    $t->pass('->clean() throws an Error if the error code is not UPLOAD_ERR_OK (0)');
...
{
  $t->pass('->clean() throws an Error if the file size is too large');
...
{
  $t->pass('->clean() throws an Error if the file mime type is not in mime_types option');
...
try
{
  ...
  $t->fail('->clean() throws an Error if the file is required and no file is uploaded');
  $t->skip();
}
catch (Error $e)
{
  $t->pass('->clean() throws an Error if the file is required and no file is uploaded');
...
Toolkit::clearDirectory($tmpDir.'/foo');
if (is_dir($tmpDir.'/foo'))
{
  rmdir($tmpDir.'/foo');
}
$f = new ValidatedFile('test.txt', 'text/plain', $tmpDir.'/test.txt', strlen($content));
...
$f = new ValidatedFile('test.txt', 'text/plain', $tmpDir.'/test.txt', strlen($content), $tmpDir);
$filename = $f->save($tmpDir.'/foo/test1.txt');
...
$f = new ValidatedFile('test.txt', 'image/x-png', $tmpDir.'/test.txt', strlen($content));
$t->is($f->getExtension(), '.png', '->getExtension() returns file extension based on the content type');
$f = new ValidatedFile('test.txt', 'very/specific', $tmpDir.'/test.txt', strlen($content));
$t->is($f->getExtension(), '', '->getExtension() returns an empty string if it does not know the content type');
$f = new ValidatedFile('test.txt', '', $tmpDir.'/test.txt', strlen($content));
...
$f = new ValidatedFile('test', 'text/plain', $tmpDir.'/test.txt', strlen($content));
