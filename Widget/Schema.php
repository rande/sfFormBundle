 ...
 */
class Schema extends Form implements \ArrayAccess
{
  const
    FIRST  = 'first',
    LAST   = 'last',
  ...
  public function getDefaults()
  {
    ...
    foreach ($this->fields as $name => $widget)
    {
      $defaults[$name] = $widget instanceof Schema ? $widget->getDefaults() : $widget->getDefault();
    }
   ...
   * @return sfWidget The current widget instance
   */
  public function addFormFormatter($name, SchemaFormatter $formatter)
  {
  ...
  public function render($name, $values = array(), $attributes = array(), $errors = array())
  {
    ...
    foreach ($this->positions as $name)
    {
      ...
      if ($widget instanceof Widget && $widget->isHidden())
      {
      ...
      else
      {
        ...
        $label = $widget instanceof Schema ? $this->getFormFormatter()->generateLabelName($name) : $this->getFormFormatter()->generateLabel($name);
        $error = $widget instanceof Schema ? array() : $error;
  ...
  public function getGlobalErrors($errors)
  {
    ...
    foreach ($this->positions as $name)
    {
      if ($this[$name] instanceof Widget && $this[$name]->isHidden())
      {
