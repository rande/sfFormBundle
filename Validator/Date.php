...
/**
 * Date validates a date. It also converts the input value to a valid date.
 *
 * @package    symfony
 * @subpackage validator
 * @author     Fabien Potencier <fabien.potencier@symfony-project.com>
 * @version    SVN: $Id: Date.class.php 26871 2010-01-19 10:43:09Z fabien $
 */
class Date extends Base
{
  ...
  protected function doClean($value)
  {
    ...
    if (is_string($value) && $regex = $this->getOption('date_format'))
    {
      ...
      {
        throw new Error($this, 'bad_format', array('value' => $value, 'date_format' => $this->getOption('date_format_error') ? $this->getOption('date_format_error') : $this->getOption('date_format')));
      }
    ...
    if ($max = $this->getOption('max'))
    {
      ...
      if ($clean > $max)
      {
        throw new Error($this, 'max', array('value' => $value, 'max' => $maxError));
      }
    }
    ...
    if ($min = $this->getOption('min'))
    {
      ...
      if ($clean < $min)
      {
        throw new Error($this, 'min', array('value' => $value, 'min' => $minError));
      }
    }
