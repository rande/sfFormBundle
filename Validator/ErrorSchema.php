...
/**
 * ErrorSchema represents a validation schema error.
 *
 * @package    symfony
 * @subpackage validator
 * @author     Fabien Potencier <fabien.potencier@symfony-project.com>
 * @version    SVN: $Id: ErrorSchema.class.php 22446 2009-09-26 07:55:47Z fabien $
 */
class ErrorSchema extends Error implements \ArrayAccess, \Iterator, \Countable
{
  protected
    $errors       = array(),
    $globalErrors = array(),
   ...
   */
  public function __construct(Base $validator, $errors = array())
  {
   ...
   * Adds an error.
   *
   * This method merges ErrorSchema errors with the current instance.
   *
   * @param Error $error  An Error instance
   * @param string           $name   The error name
   *
   * @return ErrorSchema The current error schema instance
   */
  public function addError(Error $error, $name = null)
  {
    ...
    {
      if (!isset($this->namedErrors[$name]) && !$error instanceof ErrorSchema)
      {
      ...
      else
      {
        ...
        {
          $this->namedErrors[$name] = new ErrorSchema($error->getValidator());
          $this->errors[$name] = new ErrorSchema($error->getValidator());
        }
        else if (!$this->namedErrors[$name] instanceof ErrorSchema)
        {
          $current = $this->namedErrors[$name];
          $this->namedErrors[$name] = new ErrorSchema($current->getValidator());
          $this->errors[$name] = new ErrorSchema($current->getValidator());

          $method = $current instanceof ErrorSchema ? 'addErrors' : 'addError';
          $this->namedErrors[$name]->$method($current);
          $this->errors[$name]->$method($current);
        }

        $method = $error instanceof ErrorSchema ? 'addErrors' : 'addError';
   ...
   *
   * @param array $errors  An array of Error instances
   *
   * @return ErrorSchema The current error schema instance
   */
  public function addErrors($errors)
  {
    if ($errors instanceof ErrorSchema)
    {
   ...
   *
   * @return array An array of Error instances
   */
  ...
  /**
   * Unserializes a Error instance.
   *
   * @param string $serialized  A serialized Error instance
   *
   */
